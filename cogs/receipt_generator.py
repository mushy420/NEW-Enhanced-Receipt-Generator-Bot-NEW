...
class ReceiptView(ui.View):
    ...
    async def on_store_select(self, interaction: discord.Interaction, store_id: str):
        ...
        elif store_id == "stockx":
            modal = StockXDetailsModal()
        elif store_id == "louisvuitton":
            modal = LouisVuittonDetailsModal()
        else:
            # Generic modal as fallback
            modal = GenericDetailsModal(store_id)
        
        # Store the selected store ID in the modal
        modal.store_id = store_id
...
class BaseDetailsModal(ui.Modal):
    ...
    def __init__(self, title: str = "Enter Receipt Details", timeout: int = MODAL_TIMEOUT):
        super().__init__(title=title, timeout=timeout)
        self.store_id = None
        self.receipt_data = {}
    
    async def on_submit(self, interaction: discord.Interaction):
        """Store the data from the modal fields."""
        try:
            self.receipt_data = self.gather_data()
            ...
            if validation_errors:
                ...
                embed = discord.Embed(
                    ...
                    description=f"❌ **Please fix the following issues:**\n{error_message}",
            ...
            # Create an embed to show the collected information
            embed = discord.Embed(
                title=f"{STORES[self.store_id]['name']} Receipt Details",
            ...
            for key, value in self.receipt_data.items():
                if key not in ['store_id', 'full_data']:
                    # Format the key for display
                    display_key = key.replace('_', ' ').title()
                    # Limit value length if it's too long
                    display_value = str(value)
                    if len(display_value) > 1024:
                        display_value = display_value[:1021] + "..."
                    embed.add_field(name=display_key, value=display_value, inline=False)
            
            # Create a view with edit and next buttons
            view = ReceiptDetailsView(interaction.user.id, self.store_id, self.receipt_data)
    ...
    def gather_data(self) -> Dict[str, Any]:
        """Gather data from modal fields to be overridden by subclasses."""
        return {'store_id': self.store_id}
    
    def validate_data(self) -> List[str]:
        """Validate the gathered data. Return a list of error messages."""
        errors = []
        
        # Implement common validations here
        if 'price' in self.receipt_data:
            price = self.receipt_data['price']
            if not re.match(PRICE_REGEX, price):
                errors.append("Price must be a valid number (e.g., 99.99)")
        
        if 'product_url' in self.receipt_data and self.receipt_data['product_url']:
            url = self.receipt_data['product_url']
            if not re.match(URL_REGEX, url):
                errors.append("Product URL must be a valid URL")
        
        if 'image_url' in self.receipt_data and self.receipt_data['image_url']:
            url = self.receipt_data['image_url']
            if not re.match(URL_REGEX, url):
...
class ReceiptDetailsView(ui.View):
    """View with buttons to edit receipt details or proceed to next step."""
    ...
    @ui.button(label="Edit Details", style=discord.ButtonStyle.secondary)
    async def edit_button(self, interaction: discord.Interaction, button: ui.Button):
        """Allow user to edit the receipt details."""
        if interaction.user.id != self.author_id:
        ...
        modal.prefill_fields(self.receipt_data)
    ...
    async def next_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        # Create a modal for additional customer details
        ...
        try:
            ...
            if modal.validation_errors:
                error_message = "\n".join(f"• {error}" for error in modal.validation_errors)
            ...
            # Merge the customer data with product data
            full_data = {**self.receipt_data, **modal.receipt_data}
            
            # Update the embed to show all collected information
            embed = discord.Embed(
                title=f"{STORES[self.store_id]['name']} Receipt Details",
            ...
            # Create a view with final buttons
            view = FinalReceiptView(interaction.user.id, self.store_id, full_data)
            
            await interaction.edit_original_response(embed=embed, view=view)
            
        except asyncio.TimeoutError:
            logger.warning(f"Modal timed out for user {interaction.user.id}")
            embed = discord.Embed(
                title="Timeout",
                description="The form timed out. Please start over.",
                color=discord.Color(ERROR_COLOR)
            )
...
class CustomerDetailsModal(ui.Modal):
    ...
    def __init__(self, store_id: str, timeout: int = MODAL_TIMEOUT):
        ...
        super().__init__(title=f"{store_name} Customer Details", timeout=timeout)
        self.store_id = store_id
        self.receipt_data = {}
        self.is_submitted = False
        self.validation_errors = []
        
        # Add common fields for all stores
        ...
        # Add shipping address for all stores
        self.shipping_address = ui.TextInput(
            label="Shipping Address (4 lines)",
        ...
        # Add conditional fields based on store
        if store_id in ["apple", "bestbuy", "amazon", "walmart"]:
            self.billing_address = ui.TextInput(
                label="Billing Address (can be same as shipping)",
                placeholder="123 Main St\nAnytown\n12345\nUSA",
                style=discord.TextStyle.paragraph,
                required=False
            )
            self.add_item(self.billing_address)
        
        # For shoe stores, add size
        if store_id in ["goat", "stockx"]:
            self.size = ui.TextInput(
                label="Size (can be left blank)",
                placeholder="US 10 / EU 44",
                required=False
            )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Validate the data
        self.validation_errors = self.validate_data()
        if self.validation_errors:
            # Error messages will be handled by the calling code
    ...
    def gather_data(self) -> Dict[str, Any]:
        ...
        data = {
            ...
            'shipping_address': self.shipping_address.value.strip()
        }
        
        # Add conditional fields based on their existence
        if hasattr(self, 'billing_address'):
            data['billing_address'] = self.billing_address.value.strip()
        
        if hasattr(self, 'size'):
            data['size'] = self.size.value.strip()
            
        if hasattr(self, 'condition'):
            data['condition'] = self.condition.value.strip()
    ...
    def validate_data(self) -> List[str]:
        ...
        if not re.match(DATE_REGEX, self.receipt_data['date']):
            errors.append("Date must be in MM/DD/YYYY format (e.g., 01/31/2023)")
        
        # Validate shipping address (at least has multiple lines)
        shipping_address = self.receipt_data['shipping_address']
        if shipping_address.count('\n') < 1:
            errors.append("Shipping address should have at least 2 lines")
        
        # Validate billing address if provided
        if 'billing_address' in self.receipt_data and self.receipt_data['billing_address']:
            billing_address = self.receipt_data['billing_address']
            if billing_address.count('\n') < 1:
                errors.append("Billing address should have at least 2 lines")
